...
function App() {
  ...
  const targetEyelidOpennessRef = useRef(0); // 0 = fully closed, 100 = fully open
  ...
  const updateEyelidTarget = useCallback(() => {
    ...
    // Map distance to openness: closer = more open
    // At distance 0, openness = 100%
    // At distance >= PROXIMITY_RADIUS, openness = 0%
  ...
  const animateEyelids = useCallback(() => {
    ...
    // When openness = 0: eyelids fully visible (closed)
    // When openness = 100: eyelids fully hidden (open)
    const hiddenPercent = newOpenness; // How much of the eyelid is hidden
  ...
  return (
    <div
      ref={containerRef}
      className="min-h-screen w-full bg-[#f5e0dc] flex flex-col items-center justify-center relative overflow-hidden"
    >
      {/* Eyes SVG */}
      <div className="relative mb-8">
        ...
        >
          ...
          <g clipPath="url(#mask-left)">
            {/* Moving Eye Group */}
            <g ref={leftEyeRef} className="js-eyes-left" style={{ transition: 'transform 0.1s ease-out' }}>
              {/* Normal Eye (pupil) */}
              <g className={`js-eyes-normal transition-opacity duration-300 ${isHoveringEnter ? 'opacity-0' : 'opacity-100'}`}>
                <ellipse
              ...
              <path
                className={`js-eyes-heart transition-opacity duration-300 ${isHoveringEnter ? 'opacity-100' : 'opacity-0'}`}
                fill="#FF4E1B"
            ...
            {/* Top Eyelid - PINK color matching background */}
            <rect
              ref={leftEyelidTopRef}
              className="js-eyes-eyelid-left-top"
              x="62.9"
              y="22.3"
              width="103.2"
              height="78.5"
            ...
            />
            {/* Bottom Eyelid - PINK color matching background */}
            <rect
              ref={leftEyelidBottomRef}
              className="js-eyes-eyelid-left-bottom"
              x="65.1"
              y="125.7"
              width="89.3"
              height="78.5"
          ...
          <g clipPath="url(#mask-right)">
            {/* Moving Eye Group */}
            <g ref={rightEyeRef} className="js-eyes-right" style={{ transition: 'transform 0.1s ease-out' }}>
              {/* Normal Eye (pupil) */}
              <g className={`js-eyes-normal transition-opacity duration-300 ${isHoveringEnter ? 'opacity-0' : 'opacity-100'}`}>
                <ellipse
            ...
            <rect
              ...
              className="js-eyes-eyelid-right-top"
              x="130.6"
              y="22.3"
              width="117.6"
              height="78.5"
            ...
            <rect
              ...
              className="js-eyes-eyelid-right-bottom"
              x="127.5"
              y="125.7"
              width="117.6"
              height="78.5"
      ...
      {/* Title */}
      <h1 className="text-[#5a4a42] text-2xl md:text-3xl font-normal tracking-wide mb-8">
        WELCOME BOSS
      </h1>
      ...
      <button
        ...
        className={`
          group relative px-10 py-4 rounded-full border-[2px] border-black
      ...
      <button
        className="absolute bottom-12 text-sm tracking-wider uppercase text-[#5a4a42] hover:text-black transition-colors duration-300"
        style={{
